...
async function askOpenAIWithEmbedding(question, closestText) {
    ...
    const messages = [
        { role: "system", content: "You are a helpful assistant for construction data. The current year is 2025. Please provide detailed answers, using Markdown for clear headings, bullet points, and tables. For tables, use standard Markdown table syntax. Separate major sections with '---'." },
        {
            role: "user",
...
function formatAiResponse(responseText) {
    ...
    lines.forEach(line => {
        // Check for table start/end (assuming Markdown table format)
        if (line.trim().startsWith('|') && line.includes('---')) {
            // This is a separator line for a table, indicating a table is in progress
            if (inList) { // Close any open list before table
                formattedHtml += '</ul>';
                inList = false;
            }
            if (!inTable) { // This is the header separator
                inTable = true;
                // Previous line was the header
                // Find the index of the current separator line
                const separatorIndex = lines.indexOf(line);
                if (separatorIndex > 0) {
                    tableHeader = lines[separatorIndex - 1].split('|').map(h => h.trim()).filter(h => h !== '');
                } else {
                    tableHeader = []; // No header if separator is the first line
                }
            ...
            return; // Skip separator line itself
            ...
            rowData.forEach(data => {
                formattedHtml += `<td class="px-4 py-2 thin-border-white">${data}</td>`;
            });
            formattedHtml += '</tr>';
        } else if (inTable && !line.trim().startsWith('|')) {
            // End of table (line does not start with '|')
            inTable = false;
            ...
            // H3 heading
            if (inList) { // Close any open list before heading
                formattedHtml += '</ul>';
                inList = false;
            }
            if (inTable) { // Close any open table before heading
                formattedHtml += '</tbody></table>';
                inTable = false;
            }
            ...
            // Bullet points
            if (inTable) { // Close any open table before list
                formattedHtml += '</tbody></table>';
                inTable = false;
            }
            if (!inList) {
            ...
            // Paragraph or previous list item, close list if open
            if (inList) {
                formattedHtml += '</ul>';
                inList = false;
            }
            if (inTable) { // Close any open table if a non-table line appears
                formattedHtml += '</tbody></table>';
                inTable = false;
            }
            if (line.trim() !== '') {
                 // Check if it's a link in markdown format [text](link)
                const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
                ...
                while ((match = linkRegex.exec(line)) !== null) {
                    ...
                    processedLine = processedLine.replace(fullMatch, `<a href="${linkUrl}" target="_blank" rel="noopener noreferrer" class="text-blue-400 hover:underline">${linkText}</a>`);
                }
    ...
    // Ensure any open list or table is closed at the end
    if (inList) {
        formattedHtml += '</ul>';
    }
    if (inTable) {
...
searchInput.addEventListener("keydown", async (e) => {
    if (e.key === "Enter") {
        ...
        try {
            const webResults = await searchApi(query);

            // Embedding ile en yakın sonuç bulunarak AI cevabı alınır
            const answer = await processQueryWithEmbedding(query, webResults);

            // Format the AI answer and display
            resultsContainer.innerHTML = formatAiResponse(answer);

            // Web sonuçlarını öneri olarak göster
            if (webResults.length > 0) {
                suggestionsContent.innerHTML =
                    `<p>Suggestions based on your query:</p><ul class="list-disc ml-4">` +
                    webResults
                        .map(
                            (item) =>
                ...
                suggestionsContent.innerHTML = `<p>No web suggestions available.</p>`;
            }
        } catch (error) {
